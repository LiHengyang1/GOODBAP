...
B1 = zeros(pm,pn);
B1(181:320,181 - 40:320 - 40) = 1;
B2 = zeros(pm,pn);
B2(181:320,181 + 40:320 + 40) = 1;
...
PHI1 = zeros(pm,pn);
PHI2 = zeros(pm,pn);
x = linspace(-77e-6 * 250,77e-6 * 250,pm);
x = repmat(x,pm,1);  
PHI1 = -k * x;
x = linspace(-118e-6 * 250,118e-6 * 250,pm);
x = repmat(x,pm,1); 
PHI2 = k * x;
figure;
mesh(angle(exp(1i * PHI2)))
figure;
mesh(angle(exp(1i * PHI1)))
%%
A1 = gpuArray(A1);
PHI1 = gpuArray(PHI1);
H1 = gpuArray(H1);
